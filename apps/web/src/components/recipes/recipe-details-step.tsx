...
export function RecipeDetailsStep({ initialData, onNext, onCancel }: RecipeDetailsStepProps) {
  ...
  // Auto-populate recipe name and description when product is selected
  useEffect(() => {
    if (selectedProductId && productsData?.items) {
      ...
      if (selectedProduct) {
        // Handle recipe name auto-population
        ...
        // Handle description auto-population
        const currentDescription = getValues("description")
        const expectedDescriptions = productsData.items.map(
          (p: any) => `Recipe for ${p.name}`
        )
        
        // Update description if it's empty or matches an auto-generated description
        const isDescriptionEmpty = !currentDescription || currentDescription === ""
        const isAutoGeneratedDescription = expectedDescriptions.includes(currentDescription)
        
        if (isDescriptionEmpty || isAutoGeneratedDescription) {
